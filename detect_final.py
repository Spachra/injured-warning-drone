...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, img, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            #จะส่ง line ไปยังทีมแพทย์            
            #ดูว่าส่งรูปไปไลน์ได้มั้ย #ส่งรูปได้
            
            url_Line = 'https://notify-api.line.me/api/notify'
            token = 'u4iZzAuWHY4E5UzB3bcxv9viLV5JIeFxtNvaURKywUj' # Line Notify Token
            message = 'Hello YOLO'
            data = {'message': message}
            cv2.imwrite('img2.png',im0)
            photo = 'img2.png'
            img = {'imageFile': open(photo,'rb')} #ส่งรูปได้แล้ว
            headers = {'Authorization':'Bearer ' + token}
            session = requests.Session()
            session_post = session.post(url_Line, headers=headers, files=img, data =data,)
            #session_post = session.post(url_Line, headers=headers, data =data,)
            #จบโค้ดส่งไลน์
...
def parse_opt(p):
    ...
    parser.add_argument('--source', type=str, default=ROOT / p ) #แก้ไฟลรูปที่จะใช้ detect #ทำได้แล้ว
...
while i<10 :
    ...
    token = 'HL2hTGfnH2JeSvthfTTbbmW0XE9Wf7KeHZJkR3Ef4Y4' # Line Notify Token
    photo = str(i)+".png"#ใส่เพิ่ม
    ...
    p=(str(i)+".png") #ใช้รูปเป็น defalt ในกำหนดค่าก่อน detect

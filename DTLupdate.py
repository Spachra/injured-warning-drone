...
def parse_opt(photo):
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/exp30/weights/best.pt', help='model path(s)')
    parser.add_argument('--source', type=str, default=photo) #แก้เป็นphoto
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[640], help='inference size h,w')
    parser.add_argument('--conf-thres', type=float, default=0.25, help='confidence threshold') #แก้ความแม่นยำ
...
def linenotify(message):
  ...
  img = {'imageFile': open('1.png','rb')} #Local picture File
...
while i<10 :
  ...
  yoloPhoto = yolo(photo)
  session_post = session.post(url, headers=headers, files=yoloPhoto, data =data,)
